...
def determine_kpis(
    ...
    # Bug #3 fix: only emit correlation KPIs when BOTH columns exist in enriched_profiles
    profiled_names = set(enriched_profiles.keys())
    for insight in relational_insights:
        if insight.type == 'correlation':
            col1, col2 = insight.columns[0], insight.columns[1]
            if col1 not in profiled_names or col2 not in profiled_names:
                logger.debug(f"Skipping correlation KPI for [{col1}, {col2}]: column(s) not in profiled set.")
                continue
            ...
            scored_kpis.append({
                ...
                "type": "correlation",
                "score": abs(details['correlation_coefficient'])
            })
...
def select_charts(
    ...
    # Bug #6 fix: if no categorical charts were generated (all-numeric dataset),
    # add histogram specs so the frontend always has something to show.
    if not categoricals and numerics:
        logger.info("Layer 4: No categoricals found — adding histogram fallback specs for numeric columns.")
        for p in numerics[:3]:
            spec = {
                "id": f"dist_{p.name}",
                "title": f"Distribution of {p.name}",
            ...
            # avoid duplicating if already added by the numerics loop above
            if not any(c.get("id") == spec["id"] for c in chart_specs):
